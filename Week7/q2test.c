...
void insertcq(queue *ptr,int item,int c)
{
    if(c==1)
    {
        if(ptr->f1==-1&&ptr->r1==-1)ptr->f1=0;
        ptr->r1=(ptr->r1+1)%(t+1);
        ptr->arr[ptr->r1]=item;
    }
    if(c==2)
    {
        if(ptr->f2==t&&ptr->r2==t) ptr->f2=t+1;
        ptr->r2=(ptr->r2+1)%MAX;
        ptr->arr[ptr->r2]=item;
    }    
}
...
{
    printf("\nQueue 1 Elements\n");
    for(int i=ptr->f1;i!=(ptr->r1+1);i=(i+1)%(t+1)) printf("%d \n",ptr->arr[i]);
    printf("\nQueue 2 Elements\n");
    for(int i=ptr->f2;i!=(ptr->r2+1);i=(i+1)%MAX) printf("%d \n",ptr->arr[i]);
}
