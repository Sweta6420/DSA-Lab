...
int main() {
    int n; const char programSource[2][1000] = {"__kernel void even (__global int *A) { int idx = get_global_id(0); if ((idx % 2 == 0)) A[i]=A[i]*2;}",
    "__kernel void odd (__global int *A) {int idx = get_global_id(0);if ((idx % 2 != 0)) A[i]=A[i]/2;}"};
    puts(programSource[0]); 
    puts(programSource[1]); 
    int *A = NULL;
    ...
    cl_program program = clCreateProgramWithSource(context,2,programSource,NULL,&status);
