...
void insertback(nodeptr *n,int x)
{
	...
	if(temp == NULL) *n = newnode;
		...
		while(temp->llink) temp = temp->llink;
		temp->llink = newnode;
		newnode->rlink = temp;
}}
...
int deleteback(nodeptr *n)
{
	...
	if(*n == NULL){
		printf("Underflow\n");
		return -9999;
	} else if((*n)->llink == NULL) x = (*n)->data;
	else{
		nodeptr temp = (*n);
		while((temp->llink)->llink) temp = temp->llink;
		x = (temp->llink)->data;
...
{
	nodeptr temp;
	for(temp=*n;temp;temp=temp->llink) printf("%d ",temp->data);
	printf("\n");
}
void main()
{
	...
	do {	
		...
		switch(ch) {
			case 1:
				...
				insertfront(n,ele); break;
			case 2:
				printf("Enter data : "); scanf("%d",&ele);
				insertback(n,ele);break;
			case 3:
				printf("Deleted : %d\n",deletefront(n));break;
			case 4:
				printf("Deleted : %d\n",deleteback(n)); break;
