...
void insertcq(CQ *s, Nodeptr x)
{
	...
	if((s->rear == MAX-1 && s->front == -1) || s->front == temp) printf("Queue overflow\n");
	else{
		s->rear = temp;
		s->arr[s->rear] = x;
}}
...
void levelOrder(Nodeptr root)
{
    ...
    while (temp){
        printf("%d ", temp->data);
		if (temp->lchild) insertcq(&s, temp->lchild);
		if (temp->rchild) insertcq(&s, temp->rchild);
		temp = deletecq(&s);
}}
void init1(stack *s)
{
...
void iterin(Nodeptr ptr)
{
	...
	while(ptr!=NULL){
		while(ptr!=NULL){
			if(ptr->rchild!=NULL) push(&s,ptr->rchild);
			push(&s,ptr);
			ptr = ptr->lchild;
		}ptr = pop(&s);
...
void iterpre(Nodeptr ptr)
{
	...
	while(s.top!=-1)
	{
		...
		printf("%d ",ptr->data);
		if(ptr->rchild!=NULL) push(&s,ptr->rchild);
		if(ptr->lchild!=NULL) push(&s,ptr->lchild);
}}
int main()
{
	int ch;
	Nodeptr root = NULL;
	root = create();
	printf("Inorder : ");iterin(root);
	printf("\nPostorder : ");iterpost(root);
	printf("\nPreorder : ");iterpre(root);
